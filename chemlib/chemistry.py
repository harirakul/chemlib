...
def parse_formula(formula: str) -> dict:  # Formula Parsing by Aditya Matam
    def multiply(_formula: dict, multiplier: int) -> None:
        for key in _formula:
            _formula[key] *= multiplier
...
class Compound:
    ...
    def __init__(self, formula):
        ...
        self.formula = "".join(formula).translate(SUB)
        self.coefficient = (
    ...
    def oxidation_numbers(self) -> dict:
        ...
        ox_nums[left[0]] = int(-sum([ox_nums[s] * self.occurences[s] for s in ox_nums])
                               / self.occurences[left[0]])
        return ox_nums
...
class Reaction:
    def __init__(self, reactants: list, products: list):
        ...
        self.products: List[Compound] = []
        self.compounds: List[Compound] = []
        self.reactant_formulas: List[str] = []
        self.product_formulas: List[str] = []
    ...
    def update_formula(self) -> None:
        ...
        self.coefficients = {i: formula.count(i) for i in formula}
        self.constituents = list(dict.fromkeys(formula))

        formula = []
        for i in self.constituents:
            formula.append(str(self.coefficients[i]) + i)

        del self.coefficients[" --> "]
        self.constituents.remove(" --> ")

        self.formula = " + ".join(formula).replace("+ 1 ", "").replace("  +", "")
    ...
    def limiting_reagent(self, *args, mode="grams"):
        ...
        if len(args) != len(reactants):
            raise TypeError(
                f"Expected {len(reactants)} arguments. The number of arguments "
                "should be equal to the number of reactants."
            )
    ...
    def equilibrium_concentrations(
        ...
        for i in range(len(ending_conc)):
            ...
            raise(Exception("Please raise a bug with this data: \n"
                            "Reaction.equilibrium_concentrations: %r", vars(self)))

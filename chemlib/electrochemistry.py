...
class Galvanic_Cell:
    def __init__(self, electrode1: str, electrode2: str) -> None:
        ...
        self.cell_potential = round(self.E0 = self.cathode[1][0] - self.anode[1][0], 3)
        self.line_notation = f"{self.anode[0]} | {self.anode[0]}{self.anode[1][1]}+ || {self.cathode[0]}{self.cathode[1][1]}+ | {self.cathode[0]}".replace(
            "1", ""
        )
    ...
    def draw(self, show=True):
        ...
        # Ions in Solution
        editor.text(
            (175, 327), f"{self.anode[0]}{self.anode[1][1]}+", (0, 0, 0), font=font(15)
        )
        editor.text(
            (338, 327),
            f"{self.cathode[0]}{self.cathode[1][1]}+",
            (0, 0, 0),
            font=font(15),
        )
        # Solutions
